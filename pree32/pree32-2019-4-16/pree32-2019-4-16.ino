...
void APMode()
{
	...
	if(check == 1)
	{
		...
		if (ssid.equals("%"))	//clear SSID and password if SSID is set to % 
		{
			ssid = "";
			pass = "";
		}
		...
		WiFi.begin(ssidChar,passChar); //attempt to connect
...
void printVals()
{
  ...
  Serial.print(",current:"); 	Serial.print(currentBuffer);
  Serial.print(",vpp:");		Serial.print(vpp);
  Serial.print(",voltage:");	Serial.print(voltageBuffer);
  Serial.print(",pTime1:");		Serial.print(pTime1);
  Serial.print(",pTime2:");		Serial.println(pTime2);
}
...
void preePost()
{
  ...
  digitalWrite(cLED, HIGH);
   ...
   post += relayPinString;
    ...
    http.begin("http://www.powerpree.com/methods/postpree");    //Specify destination for HTTP request     
...
void calcVals()
{ 
  currentBuffer = (ipp * currentM_float) + currentC_float; // (ipp * 0.0044) - 0.2054;
  voltageBuffer = (vpp* voltageM_float) + voltageC_float;    //  (vpp*0.1165) - 175.3;
