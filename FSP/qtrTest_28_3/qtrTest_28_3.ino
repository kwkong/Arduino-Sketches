...
void getLine()
{
	qtrVal[0] = analogRead(qtr8Pin);
	qtrVal[1] = analogRead(qtr7Pin);
	qtrVal[2] = analogRead(qtr6Pin);
	qtrVal[3] = analogRead(qtr5Pin);
	qtrVal[4] = analogRead(qtr4Pin);
	qtrVal[5] = analogRead(qtr3Pin);
	qtrVal[6] = analogRead(qtr2Pin);
	qtrVal[7] = analogRead(qtr1Pin);
...
void followRight()
{
	...
	while((strcmp(qtrState, "00111111")) && (strcmp(qtrState, "00011111")) && (strcmp(qtrState, "00001111")))
	{
		getLine();
		followStraight();
	...
	{  
		...
		while((strcmp(qtrState, "00011000")))
		{
			getLine();
			moveRight(235);
...
void followLeft()
{
	...
	if ((!strcmp(qtrState, "00000111")) || (!strcmp(qtrState, "00001111")))    //Make a Left Turn
	{  
		moveForward(80);
		delay(200);
		moveStop();
