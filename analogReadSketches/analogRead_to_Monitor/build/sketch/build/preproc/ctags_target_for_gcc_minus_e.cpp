# 1 "c:\\Arduino\\My Sketches\\analogRead_to_Monitor\\analogRead_to_Monitor.ino"
# 1 "c:\\Arduino\\My Sketches\\analogRead_to_Monitor\\analogRead_to_Monitor.ino"
...
int getVals(int analogPin, int sensorNum)
{
    ...
    if(ippCount[sensorNum] > (10 -1)) //averages every ippsSize # of loops
    {
        for(int i=0; i<10; i++) //sums all the ipps for the average
        {
            ippAvg[sensorNum] += ipps[i][sensorNum];
        }

        ippAvg[sensorNum]/= 10; //calculates average
