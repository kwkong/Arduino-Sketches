...
void getVals() 
{
    ...
    ipps[ippCount] = getIpp(sampleRate);
    ippCount++;
    ...
    {
        for(int i=0; i<replaceCount; i++)  //replaces the n biggest ipp in the array with the average
        {
            for(int i=0; i<ippsSize; i++)   //finds the biggest ipp in the array
            {
                if(ipps[i] > ippMax)
                {
                    ippMax = ipps[i];
            ...
            for(int i=0; i<ippsSize; i++)   //replaces the biggest ipp in the array with the average
            {
                if(ipps[i] == ippMax)
                {
                   ipps[i] = ippAvgOld; 
        ...
        for(int i=0; i<replaceCount; i++)  //repalces the n smallest ipp in the array with the average
        {
            for(int i=0; i<ippsSize; i++)   //finds the smallest ipp in the array
            {
                if(ipps[i] < ippMin)
                {
                    ippMin = ipps[i];
            ...
            for(int i=0; i<ippsSize; i++)   //replaces the smallest ipp in the array with the average
            {
                if(ipps[i] == ippMin)
                {
                   ipps[i] = ippAvgOld; 
...
void calcVals() 
{
    currentBuffer = (ipp * currentM_float * 1.11) + currentC_float;  //#####MADE CHANGES****
