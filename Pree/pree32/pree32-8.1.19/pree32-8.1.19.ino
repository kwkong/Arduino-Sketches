...
#define currentM 0.004
#define currentC -0.0962
#define voltageM 0.191
#define voltageC -13.7
...
void setup() 
{
 ...
 // while(1);
  ...
  ssid = preferences.getString("ssid");   // get whatever is stored in that loc
    ...
    if(WiFi.status() == WL_CONNECTED)
      {
        ...
        //inititial post 
        Serial.println("making inititial post to server");
        HTTPClient http;   
...
void preeTestMode()
{
  ...
  if(digitalRead(cButt) == 0)       //changes preetestmode
  {
    testModeMode++;
    if(testModeMode >= 7)  //kriston
      testModeMode = 0;
  
    while(digitalRead(cButt) == 0);
  }

  //kriston put a mode here
  ...
  float avgCal = 0;
   int i = 0;
    Serial.println("press sButt for avg VPP and VOLTAGE reading");
    if(digitalRead(sButt) == 0)
    {
      while(digitalRead(sButt) == 0);
      while( i < avgSize )
      ...
      avgCal = avgCal/avgSize;       //avverage 
  ...
  float avgCal = 0;
   int i = 0;
    Serial.println("press sButt for avg IPP and CURRENT reading");
    if(digitalRead(sButt) == 0)
    {
      while(digitalRead(sButt) == 0);
      while( i < avgSize )
...
//  currentBuffer = (ipp - 14.185)/218.04;
  ...
  currentBuffer = (ipp * currentM) + currentC; // (ipp * 0.0044) - 0.2054;
  voltageBuffer = (vpp* voltageM) + voltageC;    //  (vpp*0.1165) - 175.3;
